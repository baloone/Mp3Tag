...
(function() {
    ...
    const FID3 = async function(file) {
        let fileReader = await FR (file);
        const header = Header (
            await (await fileReader.slice (0, 3)).toString (),
        ...
        fileReader = await fileReader.slice (10);
        if (header.extended) throw new Error ('FID3: Extended header not implemented');
        if (header.version > 2) {
            let id = await (await fileReader.slice (0, 4)).toString ();
